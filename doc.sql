...
[]	     Representa qualquer caracter 
         que esteja escrito entre as []              h[oa]t finds hot and hat, but not hit

^	     Representa qualquer caracter 
         que NÂO esteja escrito entre as []	        h[^oa]t finds hit, but not hot and hat
...
Existem dois Wildcard (curingas) frequentemente usados em conjunto com o operador LIKE:

    (%) - O sinal percentual representa zero, um ou vários caracteres.
    (_) - O sublinhado representa um único personagem.
...
-- Os operadores ANY e ALL são usados co uma cláusila WHERE ou HAVING.
